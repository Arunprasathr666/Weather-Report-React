...
const WeatherDetails = ({icon, temp, city, country, lat, log, humidity, wind, loading, error, cityNotFound}) => {
  return(
    ...
    {cityNotFound && <div className="city-not-found">City not found</div>}
     <p className="copyright">Designed by  <span>Arun Prasath</span></p>
    </>
  )
...
function App() {
  let api_key = "572ac0e5d953964791268b0e71ac30e7";
  ...
  const search=async () => {
    ...
    try{
      ...
      console.error("An error occurrecd;",error.message);
   ...
   <>
      ...
      <div className="input-container">
        <input type="text"
         ...
         <div className="search-icon" onChange={()=>
         search()} >
          <img src={searchIcon}
          alt="Search" style= {{width: '13px', height: '14px'}}/>

         </div>
      </div>
      {!loading && !cityNotFound && <WeatherDetails  icon={icon} temp={temp} city={city} 
      country={country} lat={lat} log={log}
      humidity={humidity} wind={wind} loading={loading}
       error={error} cityNotFound={cityNotFound}/>}
      </div>
   </>
  )
}
